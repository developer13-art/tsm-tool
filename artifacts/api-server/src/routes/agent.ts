    ...
    def _api(self, method: str, path: str, **kw):
        ...
        for attempt in range(2):
            try:
                ...
                log.warning(f"Connection dropped, reconnecting... ({e})")
                try:
                    self.session.close()
                except Exception:
                    pass
                ...
                if attempt == 1:
                    log.error(f"API {method} {path}: reconnect failed")
...
router.post("/agent/heartbeat", async (req: any, res: any): Promise<void> => {
  ...
  if (body.serialNumber) {
    ...
    // Persist / update in DB so the devices page shows it
    try {
      const validMode = ["normal", "fastboot", "recovery", "brom", "edl", "download"].includes(body.mode)
        ? body.mode as "normal" | "fastboot" | "recovery" | "brom" | "edl" | "download"
        : "normal";

      const [existing_db] = await db.select({ id: devicesTable.id })
        .from(devicesTable)
        .where(eq(devicesTable.serialNumber, body.serialNumber));
      ...
      if (existing_db) {
        ...
        await db.insert(activityLogTable).values({
          type:    "device_added",
          message: `Agent registered device: ${body.brand ?? "Unknown"} ${body.model ?? ""} (${body.serialNumber})`,
        });
      }
    } catch (e) {
